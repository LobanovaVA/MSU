...
bi_list_node * bi_list::partition(bi_list_node *l, bi_list_node *h, bool (*cmp)(bi_list_node *, bi_list_node *)) 
{ 
    bi_list_node *x = h;
    bi_list_node *i = l->prev; 

    for (bi_list_node *j = l; j != h; j = j -> next) 
    { 
        if (cmp(j, x)) 
        { 
...
void bi_list::_quick_sort (bi_list_node *l, bi_list_node *h, bool (*cmp)(bi_list_node *, bi_list_node *)) 
{ 
    if (h  && l != h && l != h->next) 
    { 
        bi_list_node *p = partition(l, h, cmp); 
        _quick_sort(l, p->prev, cmp); 
        _quick_sort(p->next, h, cmp); 
    } 
} 

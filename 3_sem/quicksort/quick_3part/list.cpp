...
/*
void list::_quick_sort (list_node *first, list_node *end, list_node *prefirst, int n, bool (*cmp)(list_node *, list_node *))
{
    ...
    for(int i = 2; i < n; i+=2)  tmp = tmp -> next;
    ...
    while (first->next != end)
    {
        ...
        if (l_min < l_max)
        {
            _quick_sort (first_min, first_max, prefirst1, l_min, cmp);
            first = first_max; prefirst = prev2;
        }
        else
        {
            if (first_max != end && first_max -> next != end)
                _quick_sort (first_max, end, prev2, l_max, cmp);
...
/*
void list::_quick_sort (list_node *begin, list_node *end, int n, bool (*cmp)(list_node *, list_node *))
{

    list_node *part1, *part2, *part3, *c1, *c2, *c3, *curr, *tmp; 
    int l1 = 0, l3 = 0;
    ...
    {
        l1 = 0, l3 = 0;
        c1 = part1 = c2 = part2 = c3 = part3 =nullptr;

        tmp = (begin) ? begin -> next : head;
        for (int i = 0; i < n; i+=2)  tmp = tmp -> next;

        curr = (begin) ? begin -> next : head;
        for (int i = 0; i < n; i++)
        {
            ...
            else
            {
                ...
                {
                    if (!c1) c1 = part1 = curr;
                    else  c1 = c1 -> next = curr;
                    l1++;
                }
                else
                {
                    if (!c3) c3 = part3 = curr;
                    else c3 = c3 -> next = curr;
                    l3++;
                }
            }
            curr = curr -> next;
        ...
        if (c1) c1 -> next = part2;
        c2 -> next = part3;
        if (begin) begin -> next = (part1) ? part1 : part2;
        else head = (part1) ? part1 : part2;
        if (c3) 
        {
            c2 -> next = part3;
            c3 -> next = end;
        ...
        if (l1 < l3)
        {
            _quick_sort (begin, part2, l1, cmp);
            n = l3;
            begin = c2;
        }
        else 
        {
            _quick_sort (part2, end, l3, cmp);
            n = l1;
            end = part2;  
        }
...
list_node * list::select_elem(list_node *begin, list_node *end)
{
    double sum = 0, dif = 0, min_dif = 0;
    int n = 0, y = 0;
    list_node *curr, *elem = (begin) ? begin -> next : head;

    for(curr = (begin) ? begin -> next : head;  curr != end; curr = curr -> next)
    {
        sum += (curr -> hash());
        n++;
    }

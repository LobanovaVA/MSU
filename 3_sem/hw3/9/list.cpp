...
list_node* list::solve_1 (student& s)
{
    list_node *tmp;
    current = head;
...
list_node* list::solve_8 (int k)
{
    ...
    while (current)
    {
        ...
        while( (current -> next) && fl)
        {
            if ( (*current < *(current -> next)) || (*current > *(current -> next)) ) fl = 0;
            else { m++; current = current -> next; }
...
list_node* list::solve_9 (list_node *head_b)
{
    int fl;
    list_node *tmp, *trash, *current_a, *current_b, *finish;
    current = head;
    ...
    while (current) {
        ...
        {
            finish = current_a;
            if ( (*current_a < *current_b) || (*current_a > *current_b) ) fl = 0;
        ...
        if (fl)
        {
            if (current_a) current = current_a;
            ...
            if (!current_b) 
            {
                ...
                else
                {
                    ...
                    if (finish) 
                    {
                        tmp -> prev -> next = finish;
                        finish -> prev -> next = nullptr;
                        finish -> prev = tmp -> prev;
                    }
                    else tmp -> prev -> next = nullptr;
                    while (tmp) 
                    {

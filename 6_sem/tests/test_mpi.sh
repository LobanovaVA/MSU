...
then
	...
	do \
		...
		echo "=================== ${data_file} k = 1 ==================="
		result=`mpirun -np 1 a.out 4 3 4 0 ${data_file} | tr -d '\0'`
		parse $fl "${result}"

		echo "=================== ${data_file} k = 2 ==================="
		result=`mpirun -np 2 a.out 4 3 4 0 ${data_file} | tr -d '\0'`
		parse $fl "${result}"		
	done
	...
	do \
		...
		echo "=================== ${data_file} k = 1 ==================="
		result=`mpirun -np 2 a.out 6 3 4 0 ${data_file} | tr -d '\0'`
		parse $fl "${result}"
				...
				then
					...
					do \
						...
						then
							...
							result=`mpirun -np ${k} a.out ${n} ${m} 4 ${s} | tr -d '\0'`		
							parse $fl "${result}"
						fi
					done
				fi

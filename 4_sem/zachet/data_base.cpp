...
{ 
    switch(line.type)
    {
        ...
        case SELECT: 
        {
            if (line.par == PAR_NONE) print_select  (select (line));
            else print_distinct (select (line), line); 
            break;
        }
...
void data_base::print_distinct (list_node *res, command &line) 
{
    if (!res) return;
    conteiner.head_found = res;
    list_node * tmp, cur;
    conteiner.Neuman_sort(line);
    res = conteiner.head_found;
    //conteiner.linear_insertion_sort (line);
    while (res)
    {
        tmp = res -> next_found;
        res -> next_found = nullptr;
        if (!tmp) res -> print();
        else if (conteiner.cmp(res, tmp, line)) res -> print();
        res = tmp;
    }
}

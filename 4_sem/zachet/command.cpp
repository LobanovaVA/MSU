...
int command::parse ( int *space_alph, const char *str)
{
    ...
    if (!strcmp(readed_str + pos[0], "select"))
    {
        ...
        {
            if (!strcmp(readed_str + pos[1], "distinct"))
            {
                if (!strcmp(readed_str + pos[2], "name")) par = NAME;
                else
                {
                    if (!strcmp(readed_str + pos[2], "phone")) par = PHONE;
                    else 
                    {
                        if (!strcmp(readed_str + pos[2], "group")) par = GROUP;
                        else return  ERROR_PARSE_DISTINCT;
                    }
                }
            }
...
int like (char *str_name, char *str_like) 
{
    ...
    int len_name = strlen(str_name),  A[LEN] = {0}, B[LEN] = {0}, i; 
